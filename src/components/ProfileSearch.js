...
//     }
// ]; // move this to a seperate data file

const ProfileSearch = ({searchQuery}) => {
    const dataFile = searchQuery.map((data) => {
     return  <li key= {data.id}>
            <img src={data.picture} alt='clients profile' />
           <div>
           <p>{data.id}</p>
    ...
    const content= dataFile?.length ? dataFile : '';
  return (
    <div><ul className='profile-card'>{content}</ul></div>
  )
}
